...
export default function RiskGraph({ prompt: p }) {
  if (!p) return null
  ...
  return (
    <svg width="100%" viewBox="0 0 660 300" style={{ display: 'block' }}>
      ...
      {edges.map(([a, b]) => {
        ...
        return (
          <line key={`${a}-${b}`}
            x1={na.x + W} y1={na.y + H / 2}
            x2={nb.x} y2={nb.y + H / 2}
            stroke="#B4B2A9" strokeWidth="1"
      ...
      {nodes.map((n, i) => (
        <g key={i}>
          <rect x={n.x} y={n.y} width={W} height={H} rx="8"
            fill={n.c + '1a'} stroke={n.c} strokeWidth="1"/>
          <text x={n.x + 13} y={n.y + 13} fontSize="11">{n.icon}</text>
          <text x={n.x + W / 2} y={n.y + 15} textAnchor="middle"
            dominantBaseline="central" fontSize="11" fontWeight="500" fill={n.c}>{n.label}</text>
          <text x={n.x + W / 2} y={n.y + 30} textAnchor="middle"
            dominantBaseline="central" fontSize="10" fill={n.c} opacity="0.8">{n.sub}</text>
        </g>
      ))}

      <rect x="240" y="200" width="160" height="36" rx="8"
        fill={riskColor(pr) + '1a'} stroke={riskColor(pr)} strokeWidth="0.5"/>
      <text x="320" y="213" textAnchor="middle" dominantBaseline="central"
        fontSize="11" fontWeight="500" fill={riskColor(pr)}>Propagated: {pr.toFixed(2)}</text>
      <text x="320" y="229" textAnchor="middle" dominantBaseline="central"
        fontSize="10" fill={riskColor(pr)} opacity="0.85">
        → {p.status === 'blocked' ? 'BLOCKED' : p.status === 'approval' ? 'NEEDS APPROVAL' : 'ALLOWED'}
      </text>
    </svg>
  )
}

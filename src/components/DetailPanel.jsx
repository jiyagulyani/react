...
export default function DetailPanel({ prompt: p }) {
  ...
  const bannerStyle = p.status === 'blocked'
  ...
  const bannerTitle = p.status === 'blocked' ? 'Execution blocked'
    : p.status === 'allowed' ? 'Execution allowed' : 'Awaiting approval'
    ...
    <div style={{
      ...
      {chain.map((c, i) => (
        <div key={i} style={{
          ...
          padding: '9px 0', borderBottom: i < chain.length - 1 ? '0.5px solid rgba(0,0,0,0.08)' : 'none',
        }}>
          <div style={{
            width: 28, height: 28, borderRadius: 6, background: '#f4f4f2',
      ...
      <div style={{
        marginTop: 12, padding: 10, background: '#f4f4f2',
        borderRadius: 8, fontSize: 11,
      }}>
        <div style={{ color: '#5f5e5a', marginBottom: 3 }}>Propagation formula</div>
        <div style={{ fontSize: 10, color: '#9c9a92' }}>
          {p.risk} × {p.aw} × {p.tw} × {p.acw} × {p.sw} = {' '}
          <strong style={{ color: riskColor(pr) }}>{pr.toFixed(2)}</strong>
        </div>
      </div>
    </div>
  )
